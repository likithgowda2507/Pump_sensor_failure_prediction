...
with st.sidebar:
    ...
    with col_t2:
        st.caption("Auto-refreshing enabled")
...
with t3:
    ...
    with col3_1:
        # Use the actual failure column from dataset
        target_col = 'Failure_in_Next_6h'
        if target_col in df.columns:
            fail_stats = df.groupby('Pump_ID')[target_col].sum().reset_index()
            fig_stats = px.pie(fail_stats, values=target_col, names='Pump_ID', hole=0.6, title="Failure Distribution Across Fleet", 
                               color_discrete_sequence=px.colors.sequential.RdBu)
            fig_stats.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)")
            st.plotly_chart(fig_stats, use_container_width=True)
        else:
            st.info("Historical failure data not available for this dataset schema.")
        
    with col3_2:
        # Select pump history
        pump_hist = full_engineered[full_engineered['Timestamp'] <= current_time]
        p_fails = pump_hist[pump_hist[target_col] == 1] if target_col in pump_hist.columns else pd.DataFrame()
        
        st.markdown(f"#### Incident Log: {selected_pump}")
        if not p_fails.empty:
            # Use Total_Runtime_Hours from the 25K dataset
            runtime_col = 'Total_Runtime_Hours'
            cols_to_show = ['Timestamp']
            if runtime_col in p_fails.columns:
                cols_to_show.append(runtime_col)
            
            st.dataframe(p_fails[cols_to_show].tail(), use_container_width=True, hide_index=True)
            runtime_max = pump_hist[runtime_col].max() if runtime_col in pump_hist.columns else 0
            st.metric("Total Uptime", f"{runtime_max:,.0f} Hours")
        else:
